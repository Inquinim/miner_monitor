...
def query_scheduler(sc, var_dict):
    ...
    dt = datetime.strftime(
        datetime.now(tz=pytz.timezone(var_dict["PY_TIMEZONE"])), "%d/%m/%Y at %I:%M %p"
    )
    ...
    requests.post(
        var_dict["WEBHOOK_GLOBAL"],
        {"content": f"{var_dict['USER']}: Pinged metagraph on {dt}"},
    )
...
def query_miners(var_dict):
    ...
    neurons_below = [
        nn[0]
        for nn in neurons
        if nn[0].trust < trust_threshold and len(nn[0].bonds) == 0
    ]
    ...
    if len(neurons_below) > 0:
        ...
        send_dc_notification(
            f"Miner(s) below trust threshold: {len(neurons_below)} / {len(neurons)} on {dt}",
            webhook,
        )
